...
export default function AutomacoesEditor() {
  ...
  const initialData = useMemo(() => {
      ...
      if (template) {
        return {
          ...
          edges: template.edges.map(e => ({
            ...
            animated: true, style: { stroke: 'hsl(var(--border))' },
          })),
        };
      }
    }
    ...
    if (auto) {
      return {
        nome: auto.nome, status: auto.status,
    ...
    return { nome: t("new_automation_name"), status: 'rascunho' as const, nodes: [], edges: [] };
  }, [id, t]);
  ...
  const onConnect = useCallback((params: Connection) => {
    ...
    setEdges(eds => addEdge({ ...params, animated: true, style: { stroke: 'hsl(var(--border))' } }, eds));
  }, [nodes, t]);
  ...
  const onDrop = useCallback((event: React.DragEvent) => {
    ...
    const newNode: Node = { id: `node-${Date.now()}`, type: 'automacaoBloco', position, data: { blocoType, config: {} } };
    ...
    <div className="h-screen flex flex-col bg-background">
      <div className="h-14 border-b border-border/30 flex items-center gap-3 px-4 bg-card/50 shrink-0">
        ...
        <Badge variant={status === 'ativa' ? 'default' : status === 'pausada' ? 'secondary' : 'outline'}
          ...
          {status === 'ativa' ? t("active") : status === 'pausada' ? t("paused") : t("draft")}
        </Badge>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div ref={reactFlowWrapper} className="flex-1 relative">
          <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange}
            ...
            <MiniMap className="!bg-card !border-border/30" nodeColor={() => 'hsl(var(--primary))'} maskColor="hsl(var(--background) / 0.8)" />
          </ReactFlow>
        </div>
