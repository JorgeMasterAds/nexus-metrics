  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#F5F5F5]" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Pipeline</h1>
          <p className="text-sm text-[#A0A0A0]">
            Valor total: <span className="font-bold" style={{ color: "#E5191A" }}>{fmt(totalValue)}</span>
          </p>
        </div>
        <button
          onClick={() => setShowCreate(true)}
          className="h-9 px-4 rounded-md text-sm font-medium text-white flex items-center gap-1.5"
      ...
      {crm.isLoading ? (
        <div className="flex gap-4">
          {[1, 2, 3, 4].map(i => <div key={i} className="w-[280px] h-[300px] rounded-md bg-[#1C1C1C] animate-pulse flex-shrink-0" />)}
        </div>
      ) : crm.deals.length === 0 ? (
        <div className="flex flex-col items-center py-20">
          <Kanban className="h-12 w-12 text-[#E5191A] opacity-40 mb-4" />
          <p className="text-lg font-semibold text-[#F5F5F5]">Nenhum deal criado</p>
          <p className="text-sm text-[#A0A0A0] mb-4">Comece criando ou convertendo um lead</p>
              ...
              return (
                <div key={status.id} className="flex-shrink-0 w-[280px]" id={status.id}>
                  <div className="flex items-center gap-2 mb-3 px-1">
                    ...
                    <span className="text-xs text-[#555]">{cards.length}</span>
                    <span className="ml-auto text-xs font-bold" style={{ color: "#E5191A" }}>{fmt(stageValue)}</span>
                  </div>
                  <div className="space-y-2 min-h-[200px]">
                    {cards.map((d: any) => (
                      <div
                        ...
                        className="rounded-md p-4 border cursor-pointer transition-all hover:border-[#E5191A]"
                        style={{
                          background: "#161616",
                          borderColor: "#2A2A2A",
                      ...
                      >
                        ...
                        {d.crm2_organizations?.name && (
                          <p className="text-xs text-[#A0A0A0] mt-0.5">{d.crm2_organizations.name}</p>
                        )}
                        <p className="text-lg font-bold mt-2" style={{ color: "#E5191A", fontFamily: "'JetBrains Mono', monospace" }}>
                          {fmt(d.deal_value || 0)}
                        </p>
                    ...
                    <button
                      ...
                      className="w-full py-2 text-xs text-[#555] hover:text-[#A0A0A0] transition-colors"
                    >
                      + Adicionar deal
                    </button>
                  </div>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent style={{ background: "#161616", borderColor: "#2A2A2A" }}>
          <DialogHeader>
            <DialogTitle className="text-[#F5F5F5]">Novo Deal</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            ...
            <div>
              <label className="text-xs text-[#A0A0A0] mb-1 block">Valor (R$)</label>
              <input
                type="number"
