...
import {
  Zap, List, Play, FileText, BookOpen, ArrowLeft, Menu, X,
  ChevronRight, User, LogOut, Mail, Settings,
} from "lucide-react";
...
export default function AutomacoesLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { icon: List, label: t("automations_title"), path: "/automacoes" },
    { icon: BookOpen, label: t("automation_templates"), path: "/automacoes/modelos" },
    { icon: Play, label: t("exec_history"), path: "/automacoes/historico" },
  ];

  const pathLabels: Record<string, string> = {
    "/automacoes": t("automations_title"),
    "/automacoes/modelos": t("automation_templates"),
    "/automacoes/historico": t("exec_history"),
  };

  const { data: userProfile } = useQuery({
    queryKey: ["automacoes-user-profile"],
    queryFn: async () => {
  ...
  const isActive = (path: string) => {
    if (path === "/automacoes") return location.pathname === "/automacoes";
    return location.pathname.startsWith(path);
  };

  const isEditorPage = location.pathname.startsWith("/automacoes/editor/");
  ...
  const buildBreadcrumb = () => {
    const segments: { label: string; path: string }[] = [{ label: t("automations_title"), path: "/automacoes" }];
    const pathname = location.pathname;

    if (pathname !== "/automacoes") {
      if (pathname.startsWith("/automacoes/editor/")) {
        segments.push({ label: t("automation_editor"), path: pathname });
      } else {
        const matched = navItems.find(n => n.path !== "/automacoes" && pathname.startsWith(n.path));
        if (matched) {
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 flex items-center gap-2.5">
        ...
        <span className="text-base font-bold text-foreground tracking-tight" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
          Nexus<span className="gradient-text">Auto</span>
        </span>
        <button className="ml-auto lg:hidden" onClick={() => setMobileOpen(false)}>
          <X className="h-4 w-4 text-muted-foreground" />
        </button>
      </div>
      <p className="px-4 text-[10px] text-muted-foreground -mt-2 mb-3">{t("automations_subtitle")}</p>
  ...
  // Editor pages render full-screen without sidebar
  if (isEditorPage) {
    return <>{children}</>;
  }
